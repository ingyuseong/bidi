...
function EditBrandingScreen({ navigation, route }) {
  ...
  const [styleMenuList, setStyleMenuList] = useState(info.brandingStyles);
  ...
  const [shopOperationTime, setShopOperationTime] = useState(info.operation_time);
  const [shopBreakTime, setShopBreakTime] = useState(info.break_time);
  const [shopNumber, setShopNumber] = useState(info.shop_number);
  const [shopExtraInfoList, setShopExtraInfoList] = useState(info.extra_info);
...
export default EditBrandingScreen;
